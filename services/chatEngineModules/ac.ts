...
export function initAcSession(_h: Helpers): { state: ChatState; messages: ChatMessage[] } {
  return {
    state: 'greeting',
    messages: [],
  };
...
export function processAcMessage(session: ChatSession, userText: string, h: Helpers): ChatSession {
  ...
  switch (session.state) {
    ...
    case 'ask_details': {
      ...
      newState.hours = hasLargeJob ? PRICING.ac.minimumHours + 2 : PRICING.ac.minimumHours;
    ...
    case 'ask_extras': {
      ...
      const priceCalc = calculatePrice('ac', newState.hours, newState.selectedExtras, newState.isUrgent);
    ...
    case 'show_price': {
      ...
      if (accepted) {
        const providers = h.getProvidersForService('ac');
        const service = SERVICES.find((s: any) => s.id === 'ac')!;
