...
export function initHandymanSession(_h: Helpers): { state: ChatState; messages: ChatMessage[] } {
  return { state: 'greeting', messages: [] };
}

export function processHandymanMessage(session: ChatSession, userText: string, h: Helpers): ChatSession {
  const newMessages = [...session.messages, h.createMessage(userText, 'user')];
  let newState: ChatSession = { ...session, messages: newMessages };

  const DETAIL_RESPONSE = 'تمام يا باشا! 🛠️ قولّي الشغل المطلوب: تركيب/فك/تصليح/تعليق...';
  const TIMING_RESPONSE = 'حلو أوي يا باشا! 👍 عايز الشغل ده امتى؟ النهاردة ولا بكرة ولا يوم تاني؟';
  const EXTRAS_RESPONSE = (): string => {
    const extras = PRICING.handyman.extras;
  ...
  switch (session.state) {
    ...
    case 'ask_details': {
      ...
      newState.hours = hasLargeJob ? PRICING.handyman.minimumHours + 2 : PRICING.handyman.minimumHours;
    ...
    case 'ask_extras': {
      ...
      const priceCalc = calculatePrice('handyman', newState.hours, newState.selectedExtras, newState.isUrgent);
    ...
    case 'show_price': {
      ...
      if (accepted) {
        const providers = h.getProvidersForService('handyman');
        const service = SERVICES.find((s: any) => s.id === 'handyman')!;
