...
export function initCarpentrySession(_h: Helpers): { state: ChatState; messages: ChatMessage[] } {
  return { state: 'greeting', messages: [] };
}

export function processCarpentryMessage(session: ChatSession, userText: string, h: Helpers): ChatSession {
  const newMessages = [...session.messages, h.createMessage(userText, 'user')];
  let newState: ChatSession = { ...session, messages: newMessages };

  const DETAIL_RESPONSE = 'تمام يا باشا! 🪚 تركيب أثاث ولا تصليح ولا تفصيل؟ اديني تفاصيل أكتر.';
  const TIMING_RESPONSE = 'حلو أوي يا باشا! 👍 عايز الشغل ده امتى؟ النهاردة ولا بكرة ولا يوم تاني؟';
  const EXTRAS_RESPONSE = (): string => {
    const extras = PRICING.carpentry.extras;
  ...
  switch (session.state) {
    ...
    case 'ask_details': {
      ...
      newState.hours = hasLargeJob ? PRICING.carpentry.minimumHours + 2 : PRICING.carpentry.minimumHours;
    ...
    case 'ask_extras': {
      ...
      const priceCalc = calculatePrice('carpentry', newState.hours, newState.selectedExtras, newState.isUrgent);
    ...
    case 'show_price': {
      ...
      if (accepted) {
        const providers = h.getProvidersForService('carpentry');
        const service = SERVICES.find((s: any) => s.id === 'carpentry')!;
