...
export function initCarpetSession(_h: Helpers): { state: ChatState; messages: ChatMessage[] } {
  return { state: 'greeting', messages: [] };
}

export function processCarpetMessage(session: ChatSession, userText: string, h: Helpers): ChatSession {
  const newMessages = [...session.messages, h.createMessage(userText, 'user')];
  let newState: ChatSession = { ...session, messages: newMessages };

  const DETAIL_RESPONSE = 'تمام يا باشا! ✨ عايز تنظيف سجاد ولا كنب ولا ستائر؟ والمساحة تقريباً قد إيه؟';
  const TIMING_RESPONSE = 'حلو أوي يا باشا! 👍 عايز الشغل ده امتى؟ النهاردة ولا بكرة ولا يوم تاني؟';
  const EXTRAS_RESPONSE = (): string => {
    const extras = PRICING.carpet.extras;
  ...
  switch (session.state) {
    ...
    case 'ask_details': {
      ...
      newState.hours = hasLargeJob ? PRICING.carpet.minimumHours + 2 : PRICING.carpet.minimumHours;
    ...
    case 'ask_extras': {
      ...
      const priceCalc = calculatePrice('carpet', newState.hours, newState.selectedExtras, newState.isUrgent);
    ...
    case 'show_price': {
      ...
      if (accepted) {
        const providers = h.getProvidersForService('carpet');
        const service = SERVICES.find((s: any) => s.id === 'carpet')!;
