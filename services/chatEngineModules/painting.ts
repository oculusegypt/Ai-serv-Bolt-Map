...
export function initPaintingSession(_h: Helpers): { state: ChatState; messages: ChatMessage[] } {
  return { state: 'greeting', messages: [] };
}

export function processPaintingMessage(session: ChatSession, userText: string, h: Helpers): ChatSession {
  const newMessages = [...session.messages, h.createMessage(userText, 'user')];
  let newState: ChatSession = { ...session, messages: newMessages };

  const DETAIL_RESPONSE = 'تمام يا باشا! 🎨 عايز دهان عادي ولا ديكور؟ وكام أوضة تقريباً؟';
  const TIMING_RESPONSE = 'حلو أوي يا باشا! 👍 عايز الشغل ده امتى؟ النهاردة ولا بكرة ولا يوم تاني؟';
  const EXTRAS_RESPONSE = (): string => {
    const extras = PRICING.painting.extras;
  ...
  switch (session.state) {
    ...
    case 'ask_details': {
      ...
      newState.hours = hasLargeJob ? PRICING.painting.minimumHours + 2 : PRICING.painting.minimumHours;
    ...
    case 'ask_extras': {
      ...
      const priceCalc = calculatePrice('painting', newState.hours, newState.selectedExtras, newState.isUrgent);
    ...
    case 'show_price': {
      ...
      if (accepted) {
        const providers = h.getProvidersForService('painting');
        const service = SERVICES.find((s: any) => s.id === 'painting')!;
