...
export function initApplianceSession(_h: Helpers): { state: ChatState; messages: ChatMessage[] } {
  return { state: 'greeting', messages: [] };
}

export function processApplianceMessage(session: ChatSession, userText: string, h: Helpers): ChatSession {
  const newMessages = [...session.messages, h.createMessage(userText, 'user')];
  let newState: ChatSession = { ...session, messages: newMessages };

  const DETAIL_RESPONSE = 'تمام يا باشا! 🔌 الجهاز نوعه إيه؟ (غسالة/ثلاجة/بوتاجاز...) وإيه المشكلة؟';
  const TIMING_RESPONSE = 'حلو أوي يا باشا! 👍 عايز الشغل ده امتى؟ النهاردة ولا بكرة ولا يوم تاني؟';
  const EXTRAS_RESPONSE = (): string => {
    const extras = PRICING.appliance.extras;
  ...
  switch (session.state) {
    ...
    case 'ask_details': {
      ...
      newState.hours = hasLargeJob ? PRICING.appliance.minimumHours + 2 : PRICING.appliance.minimumHours;
    ...
    case 'ask_timing': {
      ...
      if (userText.includes('النهاردة') || isUrgent) newState.scheduledDate = 'النهاردة';
      else if (userText.includes('بكرة')) newState.scheduledDate = 'بكرة';
      else newState.scheduledDate = userText;
    ...
    case 'ask_extras': {
      ...
      const priceCalc = calculatePrice('appliance', newState.hours, newState.selectedExtras, newState.isUrgent);
      const urgentText = newState.isUrgent ? '\n⚡ رسوم خدمة عاجلة: ' + priceCalc.urgentFee + ' ج.م' : '';
      const extrasText = newState.selectedExtras.length ? '\n➕ خدمات إضافية: ' + priceCalc.extrasTotal + ' ج.م' : '';
    ...
    case 'show_price': {
      ...
      if (accepted) {
        const providers = h.getProvidersForService('appliance');
        const service = SERVICES.find((s: any) => s.id === 'appliance')!;
    ...
    case 'show_providers': {
      ...
      if (userText.includes('1') || userText.includes('١') || userText.includes('الأول')) selectedProvider = providers[0];
      else if (userText.includes('2') || userText.includes('٢') || userText.includes('التاني')) selectedProvider = providers[1];
      else if (userText.includes('3') || userText.includes('٣') || userText.includes('التالت')) selectedProvider = providers[2] || providers[0];
      else selectedProvider = providers[0];

      newState.selectedProvider = selectedProvider;
      newState.orderNumber = h.generateOrderNumber();

      const priceCalc = calculatePrice('appliance', newState.hours, newState.selectedExtras, newState.isUrgent);
      const confirmMsg = `تم اختيار ${selectedProvider.name} ✅\n\n📋 ملخص الطلب:\n🔹 الخدمة: ${SERVICES.find((s: any) => s.id === 'appliance')?.name}\n📍 التوقيت: ${newState.scheduledDate}\n💰 الإجمالي: ${priceCalc.total} ج.م\n👨‍🔧 الفني: ${selectedProvider.name}\n\nأأكد الطلب؟ قول "تأكيد" وهنبدأ على طول!`;
    ...
    case 'confirm_order': {
      ...
      if (confirmed) {
        ...
        newState.messages.push(h.createMessage(orderMsg, 'bot', 'order', { orderNumber: newState.orderNumber }));
        newState.messages.push(h.createMessage(`أنا ${newState.selectedProvider?.name}، وصلني طلبك يا باشا! أنا جاي خلال ${newState.selectedProvider?.responseTime} إن شاء الله. لو محتاج أي حاجة كلمني هنا 👋`, 'bot'));
    ...
    case 'order_created':
    case 'handover': {
      newState.messages.push(h.createMessage('تمام يا باشا، أي حاجة تانية أنا تحت أمرك! لو محتاج تعدل أي حاجة في الطلب قولي', 'bot'));
      return newState;
    }
